...
| Carrier layer | `dsh-host-webserver` (Service Definition) + `dsh-host-webserver-node` (Node provider) | Web HTTP and WebSocket routes dispatched over the Fetch standard: static serving + `/api/*`→handler forwarding + WebSocket routes + close semantics; plugin bundle endpoint + `__DSH_BOOT__` manifest injection (fed by the web plugin registry) | Web (browser access) only; the Service Definition has zero workspace dependencies (the registry arrives by structural injection); Electron does not reuse it |
