...
**传输五分。** `dsh-host-apiproxy` 是网关插件（`api-gateway` 行）：默认导出 `ApiProxyService`，只配置 `{nativeOpen?}`，消费 base 层不偏向特定入口的 `ctx.agentDefaultModel`，provide `ctx.apiProxy`，保持传输无关且不注册路由。`dsh-host-webserver` 是载体 Service Definition：抽象类 `WebServer` provide `ctx.webServer`（`register(route) → disposer`、重复 pattern 即抛、由 `fetch(request)` 分发的 Fetch 形处理器、WebSocket 路由、`renderIndex` 渲染——先结构化 `webserver/index-inject` 行、后原始 `tapIndex` 按注册序应用——与 `address`），且不认识任何 harness 概念；`dsh-host-webserver-node` 是激活即 listen、单请求失败时答 400 并记日志的 provider。connection node 半拥有从 `ctx.apiProxy` 经 `toFetchHandler` 得到的 `/api` 路由。modules node 半（`ClientModuleRegistry`，provide `ctx.clientModules`）拥有基于 `clientBundleSource` Service Definition（`dsh-client-bundle-source-node` 解析 `node_modules`）的单包增量扫描、bundle 路由、启动注入行与 `onRebuilt`/`onGraphChanged` 通知。HMR（热模块替换） node 半通过 `fs.watchFile` membership 与 `/plugins/events` SSE 路由拥有开发期重载。
