...
- **A settled mount serves until the source's stamp for the preset changes.** The composition a running session joined must survive its source changing or disappearing; each generation records the opaque `AgentPresetSource.stamp` its rows were read under (the filesystem provider's is the file's mtime + size) and a session that finds it stale starts the next generation, so source edits — the only composition editor once authoring became copy-only — reach later sessions without any authoring call dropping the pointer. An unreadable stamp serves the current generation; a first mount whose `composition()` rejects fails with `PresetMountError`. Joined sessions keep their generation, and superseded generations are reclaimed only by whole-tree teardown — deliberate, bounded by edit frequency, recorded in the package's Known Limitations.
