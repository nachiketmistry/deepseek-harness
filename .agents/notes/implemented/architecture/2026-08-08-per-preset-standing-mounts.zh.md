...
- **挂载一旦成功即持续供职，直到来源对该 preset 的 stamp 变化。** 运行中会话加入的组装必须在其来源被修改或删除后继续存活；每个代际记录其各行读取时所处的不透明 `AgentPresetSource.stamp`（文件系统提供方的是文件的 mtime + 大小），会话发现当前代际已陈旧时，会开启下一个代际，因此来源编辑——创作改为仅复制之后唯一的组装编辑器——无需任何创作调用丢弃指针即可达到后续会话。stamp 无法读取时继续提供当前代际；首次挂载时 `composition()` reject 则以 `PresetMountError` 失败。已加入的会话保持其代际，被替代的代际只由整树卸载回收——刻意为之，上限取决于编辑频率，已记入包的 Known Limitations。
