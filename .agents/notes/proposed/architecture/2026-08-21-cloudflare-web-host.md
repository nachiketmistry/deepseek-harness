...
1. **Gate 0** (`apps/cf-web`): bundle the web composition for workerd and evaluate every CF-target package module inside workerd. Shipped: the complete tree bundles with zero unresolved imports at 0.98 MiB gzip; the CF target composition (107 package rows) is 0.37 MiB gzip and every module evaluates under local and production workerd. Production workerd forbids `new Function` and leaves `import.meta.url` undefined, which the tree now avoids on its module-load path (self-package JSON imports, a lazy `!!js` evaluator); `dsh-skill-badge` stays excluded until an asset-backed provider replaces its disk assets. No structural blocker: every remaining Node coupling is call-time.
