...
const lines = [
  ...
  '',
  `${[...gaps.keys()].length} capability gaps, ${openGaps.length} of them open. ${host.ledger.filter(r => r.kind === 'replaced').length} host rows are replaced by a Cloudflare provider and ${host.ledger.filter(r => r.kind === 'not-applicable').length} do not apply to this deployment.`,
  '',
  '## Capability gaps',
  '',
