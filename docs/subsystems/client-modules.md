...
A package joins the table by declaring `dsh.client` (`platform: 'web'`, optional `inject` edges, optional `immediately`) in its package.json and exporting its built bundle at `exports["./client"]`. The registry reads declarations and bundle bytes only through the `clientBundleSource` Service Definition (`ClientBundleSource`: `describe`, `read`, `readSourceMap`, `locate`). The Node provider [dsh-client-bundle-source-node](../../packages/client/bundle-source-node) anchors package resolution at the config tree's `ctx.baseUrl` — the cordis.yml directory, whose package declares every composed plugin as a dependency — and its construction throws when that anchor is unset; a platform host provides the same contract over a build-time manifest.
