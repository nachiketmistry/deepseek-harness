...
The Host API requires the same browser session the Node surface requires, and the index response is where a launch token is exchanged for it. A Worker has no terminal, so the handoff line naming that URL goes to the deployment's log stream once per isolate that mints a token; the token is per-isolate, so an operator reads it from a cold boot rather than expecting it to persist. The cookie's signing secret is durable in the Cloudflare credential store, so sessions survive isolate restarts even though tokens do not.
