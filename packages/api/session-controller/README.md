...
Creating a Session ensures its project directory exists through the filesystem seam, so the controller injects `fs` and a deployment composing it without a provider fails to activate rather than failing on the first chat. Where those files are is the provider's answer, not this package's: on Cloudflare they are in a sandbox container the Worker cannot reach with `node:fs` at all.
