...
创建 Session 时会经由文件系统接缝确保其项目目录存在，因此本控制器注入 `fs`；组合了它却没有 Provider 的部署会在激活时失败，而不是在第一次对话时失败。这些文件位于何处由 Provider 作答，而非本包：在 Cloudflare 上它们位于一个 Worker 用 `node:fs` 根本无法抵达的 sandbox 容器中。
