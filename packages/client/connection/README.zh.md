...
- **`/api` 路由把每个请求体整体缓冲在内存里**：`withBodyLimit` 对超过 `maxRequestBodyBytes`（默认 160 MiB，按默认 100 MiB 图片总量上限经 base64 膨胀加信封余量得出）的请求体以 413 拒绝，否则在分发前缓冲它，因此该上限同时是单请求的驻留内存上界；要降低它而不缩小图片限额，需要流式请求体路径。
