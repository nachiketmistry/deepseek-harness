...
import AgentPresets, { type Config } from '@deepseek-ai/dsh-agent-presets'
import FilesystemAgentPresetSource, {
  SHIPPED_PRESET_ROOT, type Config as SourceConfig,
...
/** Boot a roster over the filesystem source with its shipped root left to the default. */
async function roster(config: Partial<SourceConfig> = {}): Promise<Context> {
  ...
  await ctx.plugin(AgentPresets, { default: 'standard' } satisfies Config)
  return ctx
}
...
describe('the shipped preset root', () => {
  ...
  it('prepends the shipped root before configured roots and the derived user root', async () => {
    ...
    expect((ctx.agentPresetSource as FilesystemAgentPresetSource).roots.map(root => root.path)).toEqual([
      SHIPPED_PRESET_ROOT,
      SYSTEM_ROOT,
      expect.stringContaining('.agent-presets'),
    ])
    expect((ctx.agentPresetSource as FilesystemAgentPresetSource).roots[0]).toEqual({ path: SHIPPED_PRESET_ROOT, trust: 'system' })
  ...
  it('mounts a roster without the shipped set when includeShippedRoot is false', async () => {
    ...
    expect((ctx.agentPresetSource as FilesystemAgentPresetSource).roots).toEqual([{ path: SYSTEM_ROOT, trust: 'system' }])
    const minimal = (await ctx.agentPresets.list()).find(preset => preset.id === 'minimal')
